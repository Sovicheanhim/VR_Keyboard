...
Shader "Hidden/Panorama180View/panoramaSphereRendering"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			int _TransitionType;    // 0 : 遷移しない(No Transition)、 1 : フェードイン(Fade in)、 2 : フェードアウト(Fade out)、 3 : ブレンド(blend).
			sampler2D _DestTex;  	// 状態遷移時の移行先のテクスチャ(Destination texture during state transition).
			float _TPos;		 	// 遷移の移行値 (0.0 - 1.0). (Transition value of transition)
			float4 _FadeInColor; 	// フェードインの色. (Fade-in color)
